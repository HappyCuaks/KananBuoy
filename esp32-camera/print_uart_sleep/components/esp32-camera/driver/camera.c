...
static esp_err_t dma_desc_init()
{
    ...
    size_t buf_size = line_size; /* 640 * 2 * 2 = 2560 bytes <- RGB565 */
    while (buf_size >= 4096) {
        buf_size /= 2;
        dma_per_line *= 2;
    }
    size_t dma_desc_count = dma_per_line * 4; /* 1 * 4 = 4 <- RGB565 */ 
    ...
    for (int i = 0; i < dma_desc_count; ++i) {
        ...
        if (s_state->sampling_mode == SM_0A0B_0B0C &&
                (i + 1) % dma_per_line == 0) {
            pd->length -= 4; /* I = ? -> 2560 - 4 = 2556 */
        }
        dma_sample_count += pd->length / 4; /* 2556 */
        pd->size = pd->length; /* 2556 */
        pd->owner = 1;
        pd->sosf = 1;
        pd->buf = (uint8_t*) buf;
        pd->offset = 0;
